...
%%% LONGITUTE %%%
\tdplotsetrotatedcoords{\Vthe+\Vazi}{90+\Vpol}{0}
\path[tdplot_rotated_coords,very thin,spath/save=pathname] (0,0,0) circle(1);
\path[draw,blue,very thin,spath/use={pathname, transform={shift={({0},{0},{0})}}}];
%%% LATITUTE %%%
\tdplotsetrotatedcoords{\Vazi}{0}{0}
\path[tdplot_rotated_coords,very thin,spath/save=pathname] (0,0,0) circle [radius={cos(\Vthe)}];
\path[tdplot_rotated_coords,draw,black,very thin,spath/use={pathname, transform={shift={({0},{0},{sin(\Vthe)})}}}];
}
\end{tikzpicture}
\end{frame}
\end{document}"""
