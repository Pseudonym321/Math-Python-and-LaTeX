...
\def \r {1.75}
\draw[-latex,red] (0,0,0) -- (0,0,{\r*cos(\mytheta)*(4-2*\r*sin(\mytheta)+(\r)^2*sin(\mytheta)*cos(\mytheta))}) node[pos=1,left]{$\va{c}$};

\draw[tdplot_screen_coords,white] (-3.5,-6) rectangle (5.5,6);
...
for i in np.linspace(0,180,numiter//2):
...
for i in np.linspace(180,0,numiter//2):
    # Create a TeX file for each iteration
    tex_file = 'file_{}.tex'.format(1)
    string = make_tikz_code(i)
    with open(tex_file, 'w') as f:
